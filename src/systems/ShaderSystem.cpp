...
bool ShaderSystem::initialize() {
    ...
    lightingShader = LoadShader("../resources/lighting.vs", "../resources/lighting.fs");
    
    if (lightingShader.id == 0) {
        std::cout << "Warning: Failed to load custom shaders, using default shader" << std::endl;
        return false;
    }
    
    // Get shader locations for uniforms - cache them for performance
    lightingShader.locs[SHADER_LOC_MATRIX_MVP] = GetShaderLocation(lightingShader, "mvp");
    lightingShader.locs[SHADER_LOC_MATRIX_VIEW] = GetShaderLocation(lightingShader, "matView");
    lightingShader.locs[SHADER_LOC_MATRIX_PROJECTION] = GetShaderLocation(lightingShader, "matProjection");
    ...
    SetShaderValue(lightingShader, sunDirectionLoc, &sunDirection, SHADER_UNIFORM_VEC3);
    SetShaderValue(lightingShader, sunColorLoc, &sunColor, SHADER_UNIFORM_VEC3);
    SetShaderValue(lightingShader, ambientStrengthLoc, &ambientStrength, SHADER_UNIFORM_FLOAT);
...
void ShaderSystem::enableDynamicSun(bool enable) {
    ...
    float timeOfDay = fmod(gameTime * 0.1f, 1.0f); // One full cycle every 10 seconds for demo
...
void ShaderSystem::setTimeOfDay(float normalizedTime) {
    ...
    // normalizedTime: 0.0 = midnight, 0.25 = sunrise, 0.5 = noon, 0.75 = sunset, 1.0 = midnight
    ...
    if (normalizedTime < 0.25f || normalizedTime > 0.75f) {
        ...
        float nightIntensity = 0.1f + 0.2f * (1.0f - abs(normalizedTime - 0.5f) * 2.0f);
        newSunColor = (Vector3){0.2f * nightIntensity, 0.3f * nightIntensity, 0.8f * nightIntensity};
